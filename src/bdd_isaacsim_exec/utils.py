...
def create_rigid_prim_in_scene(
    scene: IsaacScene, ns_manager: NamespaceManager, obj_model: ObjectModel, prim_prefix: str
) -> RigidPrim:
    id_str = obj_model.id.n3(namespace_manager=ns_manager)
    id_str = get_valid_var_name(id_str)

    # TODO(minhnh): handle initial poses

    obj_configs = obj_model.get_attr(key=URI_SIM_PRED_HAS_CONFIG)
    # assert obj_configs is not None and isinstance(obj_configs, dict), f"no configs for {obj_model.id}"
    if obj_configs is None:
        obj_configs = {}
    ...
    if URI_TYPE_USD_FILE in obj_model.model_types:
        assert (
            URI_SIM_TYPE_RES_PATH in obj_model.model_types
        ), f"object '{obj_model.id}' has type '{URI_TYPE_USD_FILE}' but not type '{URI_SIM_TYPE_RES_PATH}'"

        asset_path = None
        for path_model_id in obj_model.model_type_to_id[URI_SIM_TYPE_RES_PATH]:
            asset_path = obj_model.models[path_model_id].get_attr(key=URI_SIM_PRED_PATH)
            if asset_path is not None:
                break
        assert (
            asset_path is not None
        ), f"attr '{URI_SIM_PRED_PATH}' not loaded for object model '{obj_model.id}'"

        usd_model_uris = obj_model.model_type_to_id[URI_TYPE_USD_FILE]
        ...
        if URI_SIM_TYPE_ISAAC_RES in obj_model.model_types:
            ...
            raise RuntimeError(
                f"unhandled types for USD model(s) '{usd_model_uris}': {obj_model.model_types}"
            )
    ...
    if URI_PY_TYPE_MODULE_ATTR in obj_model.model_types:
        ...
        for model_id in obj_model.model_type_to_id[URI_PY_TYPE_MODULE_ATTR]:
            python_cls = import_attr_from_model(model=obj_model.models[model_id])
            if issubclass(python_cls, RigidPrim) or issubclass(python_cls, Articulation):
                correct_cls = python_cls
                break
        assert (
            correct_cls is not None
        ), f"'{obj_model.id}' has no handled Python class model: {obj_model.models.keys()}"

        return scene.add(correct_cls(name=obj_name, prim_path=prim_path, **obj_configs))

    raise RuntimeError(f"unhandled types for object'{obj_model.id}': {obj_model.model_types}")
